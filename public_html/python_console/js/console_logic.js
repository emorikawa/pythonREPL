...
function refocus()
{
 ...
 /* Setup and grab basic elements to fill in our global variables*/
function init()
{
  _in = document.getElementById("input");
  _in.style.height = "16px";
...
function recalculateInputHeight(new_height)
{
	_in.style.height = new_height;
...
function inputKeydown(e) {
  ...
  if (e.shiftKey && e.keyCode == 13) { // shift-enter
	...
	recalculateInputHeight(new_height)
	  ...
	  // Let alt set a carriage return as well;
	 new_height = parseInt(_in.style.height.replace("px","")) + 16;
	recalculateInputHeight(new_height)
	  insertAtCaret("input", "\n");
    ...
    if (e.ctrlKey || caretInLastLine(_in)) {
	  ...
	  setTimeout("recalculateInputHeight(_in.scrollHeight);",10);
  		...
  		if (language == "python") {
			// The tab behavior when executing python
			
			//return true;
			// this is default shell behaviour, which we don't want
			if (_in.value == "") {
				return true;
			} else {
			...
			// The tab behavior when executing javascript
...
{	
  if((s=String(s)))
  { 
  	var newBeforeDiv = document.createElement("div");
	newBeforeDiv.className = type + "_before before_line";
	_out.appendChild(newBeforeDiv);
  
  	// Replace control characters with html safe ones
	s = htmlEncode(s, true, 4);
...
//	s = s.replace(/\s/g,'&nbsp;');
  	...
  	if (type == "input") {
		...
		if (language == "python") {
			newpre.className = "sh_python";
		} else if (language == "javascript") {
			newpre.className = "sh_javascript";
		}
		newpre.innerHTML = s;
	...
	_out.appendChild(clearDiv);
	
	// Scroll to the bottom after we print lines
	$("#shell").scrollTop($("#shell")[0].scrollHeight);
...
function go(s)
{
  ...
  try { ("Shell" in _win) }
    ...
    printError("The Shell cannot access variables in the target window.  The most likely reason is that the target window now has a different page loaded and that page has a different hostname than the original page.");
    return;
  }
  ...
  if (!("Shell" in _win))
	...
	if (language == "python") {
		...
		$.post(
			"http://pythonrepl.appspot.com",
			{
				statement: Shell.question,
				session: mysid
			},
			function(answer){
				split_answer = answer.split('\n')
				if (split_answer[0].slice(0,4) == "sid:") {
					mysid = split_answer[0].slice(4,split_answer[0].length);
				}
				
				split_answer = split_answer.slice(1, split_answer.length);
				answer = split_answer.join('\n');
				
				Shell.printAnswer(answer);
		});
	} else if (language == "javascript") {
...
$(document).ready(function() {
	...
	/*
	$("#loading_line").ajaxStart(function(){
		console.log("AJAX Start");
		$(this).show();
	});
	$("#loading_line").ajaxComplete(function(){
		console.log("AJAX Complete");
		$(this).hide();
	});
	*/
	$(".shell-clickable").click(function() {
		insertAtCaret("input",$(this).html());
		try { go(); } catch(er) { alert(er); };
	});
